...
void WifiManagerParamHelper::Init(uint16_t preamble, const ParamEntry* entries, size_t entries_len) {
  ...
  wm_.setRunNonblockingAfterConnection(true);
  ...
  for (size_t i = 0; i < entries_len; i++) {
    if (eeprom_length >= current_size + entries[i].max_len) {
      const char *loaded_data = reinterpret_cast<const char *>(
          EEPROM.getConstDataPtr() + current_size);
      ...
      parameters_.emplace_back(entries[i].id, entries[i].label, "",
                                entries[i].max_len);
...
void WifiManagerParamHelper::OnParamCallback() {
  ...
  for (const auto &param : parameters_) {
    ...
    if (strncmp(reinterpret_cast<const char *>(EEPROM.getConstDataPtr() +
                                                current_size),
